...
(defcustom tla+-tlc-difftrace
  "-difftrace" ;; bool
  "Tell `TLC' to print only state diffs"
  :type 'string
  :group 'tla+)
...
(defcustom tla+-tlc-view
  "-view" ;; boolean
  "Tell `TLC' to display the state using the view function"
  :type 'string
  :group 'tla+)

(defcustom tla+-tlc-df
  "-dfid" ;; int
  "Tell `TLC' to do depth-first exploration starting with depth N"
  :type 'string
  :group 'tla+)
...
3. errors are converted into clickable buttons by using the
  ...
  (let* ((filename (buffer-file-name))
    ...
    (save-excursion
      (with-current-buffer
          (switch-to-buffer-other-window sanybuffer)
        (erase-buffer)
        (insert output)
        (tla+/find-error-marks filename)))))
...
(setq tla+/last-spec-checked nil)
(setq tla+/last-config-checked nil)

(setq tla+/options
      `((tla+-tlc-deadlock ,tla+-tlc-deadlock nil booleanp)
        (tla+-tlc-checkpoint ,tla+-tlc-checkpoint nil integerp)
        (tla+-tlc-difftrace ,tla+-tlc-difftrace nil booleanp)
        (tla+-tlc-df ,tla+-tlc-df nil integerp)
        (tla+-tlc-recover ,tla+-tlc-recover nil stringp)
        (tla+-tlc-simulate ,tla+-tlc-simulate nil booleanp)
        (tla+-tlc-depth ,tla+-tlc-depth 1000 integerp)
        (tla+-tlc-view ,tla+-tlc-view nil booleanp)
        (tla+-tlc-workers ,tla+-tlc-workers 2 integerp)))

(defun tla+/parse-option-value (value type)
  (cond ((eq type 'stringp) value)
        ((eq type 'integerp) (string-to-number value))
        ((eq type 'booleanp)
         (if (equal value "nil") nil t)) ))

(defun tla+/format-value (param)
  (destructuring-bind (flag value type) param
    (tla+/format-tlc-cli-option-value flag value type)))

(defun tla+/format-tlc-cli-option-value (flag value type)
    (if value
        (if (eq type 'booleanp)
            (concat flag " ")
...
(defun tla+/tlc-cli-options (&optional tlcconf do-not-reload)
  (let* ((tlcconf (or tlcconf (tla+/tlcopt-file-name)))
         (_ (or do-not-reload (tla+/read-options tlcconf)))
         (tla+/options tla+/options))
    ;; (if (tla+/get-option 'tla+-tlc-simulate)
    ;;     (concat tla+-tlc-simulate " ")
    (when (assq 'tla+-tlc-df tla+/options)
      (setq tla+/options (assq-delete-all 'tla+-tlc-workers tla+/options)))
    (mapconcat
     (lambda (x)
       (destructuring-bind (key flag def-value type) x
         (if-let ((value (tla+/get-option key)))
             (tla+/format-tlc-cli-option-value flag value type)
           (tla+/format-tlc-cli-option-value flag def-value type))))
     tla+/options
     ""))
  )

(defun tla+/tla-file-path (&optional file)
  (let ((file-name (or file tla+/last-spec-checked (buffer-file-name))))
    (concat
     (file-name-sans-extension file-name)
     ".tla" )))

(defun tla+/tla-file-name (&optional file)
  (file-name-nondirectory (tla+/tla-file-path file)))

(defun tla+/cfg-file-path (&optional file)
  (let ((file-name (or file tla+/last-spec-checked (buffer-file-name))))
    (concat
     (file-name-sans-extension file-name)
     ".cfg" )))

(defun tla+/cfg-file-name (&optional file)
  (file-name-nondirectory (tla+/cfg-file-path file)))

(defun tla+/tlcopt-file-path (&optional file)
  (let ((file-name (or file tla+/last-spec-checked (buffer-file-name))))
    (concat
     (file-name-sans-extension file-name)
     ".tlcopt" )))

(defun tla+/tlcopt-file-name (&optional file)
  (file-name-nondirectory (tla+/tlcopt-file-path file)))

(defun tla+-toggle-flag-file ()
  (interactive)
  (find-file-other-window (tla+/tlcopt-file-name)))

(defun tla+-unset-tlc-flag (opt)
  (interactive
   (progn
     (tla+/read-options (tla+/tlcopt-file-name))
     (let* ((options (mapcar (lambda (k) (symbol-name (car k))) tla+/options))
            (opt (intern (completing-read "TLC flag: " options nil t))))
         ;; (init-value (tla+/coalesce (tla+/get-option flag) (assoc flag tla+/options)))
       (list opt))))
  (let ((tlcopt (tla+/tlcopt-file-name))
        (value nil))
    (destructuring-bind (_ flag def-value type) (assoc opt tla+/options)
      (setq tla+-option-list (assoc-delete-all opt tla+-option-list))
      (unless (equal value def-value)
        (add-to-list 'tla+-option-list (cons opt value)))
      (tla+/write-options tlcopt))))

(defun tla+-set-tlc-flag (opt value)
  (interactive
   (progn
     (tla+/read-options (tla+/tlcopt-file-name))
     (let* ((options (mapcar (lambda (k) (symbol-name (car k))) tla+/options))
            (opt (intern (completing-read "TLC flag: " options nil t)))
            (value (tla+/get-option opt)))
       (destructuring-bind (_ flag def-value type) (assoc opt tla+/options)
         ;; (init-value (tla+/coalesce (tla+/get-option flag) (assoc flag tla+/options)))
         (let*
            ((choices (mapcar 'prin1-to-string
                              (seq-filter
                               'identity
                               (list value
                                     def-value))))
             (_ (message "A"))
             (str-value (read-string (format "%s = " opt)
                                     (car choices)) )
             (value (tla+/parse-option-value str-value type)))
           (if (funcall type value)
               (list opt value)
             (error "wrong value type: %s, %s" type value)))))))
  (let ((tlcopt (tla+/tlcopt-file-name)))
    (destructuring-bind (_ flag def-value type) (assoc opt tla+/options)
      (setq tla+-option-list (assoc-delete-all opt tla+-option-list))
      (unless (equal value def-value)
...
(aio-defun aio-start-file-process-shell-command (name buffer command &optional sentinel-a)
  (when-let ((buf (get-buffer buffer)))
    (with-current-buffer buf
      (when (and (boundp 'process) (process-live-p process))
        (kill-process process) )) )
  (let ((process (start-file-process-shell-command name buffer command))
        (sentinel-b (aio-make-callback)))
    ;; (message "cmd: %s\nprocess: %s" command process)

    (with-current-buffer (get-buffer buffer)
      (setq-local process process)
      ...
      (setq-local sentinel-list (list (car sentinel-b) sentinel-a))
      (set-process-sentinel
       process
       (lambda (process event)
         (with-current-buffer
             (process-buffer process)
           (dolist (s sentinel-list)
             (funcall s process event))))) )
    (aio-await (cdr sentinel-b))))

;; compile is hard to use because file names are not parsed directly
;; (extension is mission)
(defun tla+-run-model (cfgfile &optional do-not-reload)
...
"
  (interactive
     (list (read-file-name
            ...
            nil nil
            (tla+/cfg-file-name (buffer-file-name)) )))
  (if cfgfile
      (setq cfgfile (file-name-nondirectory cfgfile))
    (setq cfgfile (tla+/cfg-file-name)))
  (if (not tla+-tlatools-path)
      (error "need the TLA+ toolbox java archive: tla+-tlatools-path"))
  (let* ((filename (buffer-file-name))
        ...
        ;            (format "*tlc2.TLC* [%s]" (buffer-name))))
         (tlcbuffer (get-buffer-create tlcbufname))
         (cubuffer  (current-buffer))
         ;; XXX: tlc2.TLC -config /full/path/to/TLA.cfg TLA.tla
         ;;  not sure why the tla file itself should not be a
         ;;  full path?
         (finame (tla+/tla-file-name filename))
         ;; (tlcconf (tla+/tlcopt-file-name))
         ;; (_ (message "tlcconf"))
         ;; (loadconf (tla+/read-options tlcconf))
         (newbuf (get-buffer-create tlcbufname))
         ;; (_ (message "tlcconf"))
         (short-cmd
          (concat " tlc2.TLC "
                  ;; " -dump dot states.dot "
                  (tla+/tlc-cli-options nil do-not-reload)
                  "-config " cfgfile " "
                  finame))
         ;; (_ (message "tlcconf"))
         (cmd (concat
         ...
         ;; (_ (message "tlcconf"))
         (output (progn
                   (with-current-buffer tlcbuffer
                     (setq-local file-name finame)
                     (setq buffer-read-only nil)
                     (erase-buffer)
                     (insert short-cmd "\n\n")
                     (goto-char (point-max))
                     (setq buffer-read-only 't))
                   (aio-start-file-process-shell-command "*tlc*" tlcbuffer cmd
                                                         'tla+-tlc-sentinel)))
         ;; (output (shell-command-to-string cmd))
         (dot    (tla+-run-dot)))
    (message "end")
      (setq tla+/last-config-checked cfgfile)
      (setq tla+/last-spec-checked finame)
      ;; (with-selected-window (selected-window)
      (progn
        (pop-to-buffer tlcbuffer nil 'norecord))
      (message (concat "Ran cmd: " cmd))
      output))

'(defun tla+-edit-options ()
  (interactive)
  (tla+/read-options (tla+/tlcopt-file-path))
  (let ((cfgname (tla+/cfg-file-name)))
    (switch-to-buffer (format "*TLC Options* [%s]"
                              cfgname))
    (kill-all-local-variables)
    (erase-buffer)
    (setq buffer-read-only nil)
    (dolist (v tla+/options)
      (destructuring-bind (key flag def-val type) v
        (make-local-variable key)
        (let ((val (or (cdr (assoc key tla+-option-list))
                       def-val)))
          (set key val)
          (cond ((eq type 'booleanp)
                 (widget-insert (format "\n%s:  " key))
                 (widget-create 'checkbox
                                :notify (lambda (&rest ignore) (set key (not (symbol-value key))))
                                val))
                ((eq type 'integerp)
                 (widget-create 'editable-field
                                :format (concat (format "\n%s:  " key) "%v")
                                :value (prin1-to-string val)
                                :value-set (lambda (widget value) (set key (string-to-integer value)))
                                ))
                ((eq type 'stringp)
                 (widget-create 'editable-field
                                :format (concat (format "\n%s:  " key) "%v")
                                :value val
                                :value-set (lambda (widget value) (set key value))
                                ))))
          ))
    (setq buffer-read-only t)
    )
...
(defun tla+-tlc-sentinel (process event)
  ;; (save-excursion
  (goto-char (point-min))
  (setq buffer-read-only nil)
  (tla+/find-error-marks file-name)
  (setq buffer-read-only 't))
...
(defconst tla+/error-rx-short
  (concat "line \\([0-9]+\\), "
          "\\(col\\|column\\) \\([0-9]+\\)"))

(defconst tla+/error-rx-long
             (concat
              "line \\([0-9]+\\), " ; line XYZ
...
(defconst tla+/error-rx
  (concat "\\(" tla+/error-rx-long "\\|" tla+/error-rx-short "\\)"))
...
"
  (let* ((pos (point))
         ...
         (def-file (get-text-property (point) 'default-file))
         (text (button-label the-button))
         (s (string-match tla+/error-rx
                          text))
         (line (string-to-number (or (match-string 2 text)
                                     (match-string 10 text))))
         (col (string-to-number (or (match-string 4 text)
                                    (match-string 12 text))))
         (end-line (match-string 5 text))
         (end-col (match-string 7 text))
         (end-line (and end-line (string-to-number end-line)))
         (end-col (and end-col (string-to-number end-col)))
         (filename (match-string 9 text))
         (file (if filename
                   (find-file-other-window (concat filename ".tla"))
                 (find-file-other-window def-file)))
                 ;; (pop-to-buffer buffername)))
         )
    ;; (message "loc: %s" (list buffername filename (list line col) (list end-line end-col)))
    ;(message (format "Button pressed: %s" text))
    (progn
      (with-current-buffer file
        (goto-char (point-min))
        (when (and tla+/select-error-region
                   end-line end-col)
          (goto-line end-line)
...
(defvar tla+/select-error-region t)

(defun tla+/make-link-button (begin end def-file)
...
"
  (set-text-properties begin end
                       `(default-file ,def-file))
  (make-button begin end
               :type 'tla+/sany-button-pressed))

(setq tla+/tlc-last-checkpoint nil)

(defun tla+-unset-checkpoint ()
  (interactive)
  (tla+-unset-tlc-flag 'tla+-tlc-recover))

(defun tla+-use-latest-checkpoint (&optional args)
  (interactive "P")
  (unless tla+/tlc-last-checkpoint
    (error "no checkpoint found"))
  (if args
      (tla+-set-tlc-flag 'tla+-tlc-recover (cadr tla+/tlc-last-checkpoint))
    (tla+-set-tlc-flag 'tla+-tlc-recover (car tla+/tlc-last-checkpoint)))
  )

(defun tla+/find-error-marks (def-file)
...
"
  (progn
    ...
    (let* ((error-re (concat
                      ...
                      "\\)"
                      "\\)"))
           (re (rx (or (regex error-re)
                       (seq "Checkpointing of run "
                            (group (: "states/" (0+ not-newline)))
                            "\nCheckpointing completed at")
                       (group (: line-start "Error:")) )))
           (found-error)
           (last-line-num))
      (setq tla+/tlc-last-checkpoint nil)
      (while (re-search-forward re
        ...
        (let* ((rstart (match-beginning 0))
               (linenum (match-beginning 3))
               (colnum (match-beginning 5))
               (filename (match-beginning 10))
               (rend (match-end 0))
               )
          ;; (message "%s, %s %s" filename linenum colnum)
                                        ;(message "%d %d" rstart rend)
          ;; (message "match-list: %s" (match-list))
          (if-let ((file (match-string 14)))
              (setq def-file file)
            (if-let ((checkpoint (match-string 15)))
                (add-to-list 'tla+/tlc-last-checkpoint checkpoint)
              (if (match-string 16)
                  (setq found-error t)
                (setq last-line-num rstart)
                (tla+/make-link-button rstart rend def-file))))
          ))
      (when last-line-num (goto-char last-line-num))
      (if (and found-error tla+-auto-recover)
          (if (cadr tla+/tlc-last-checkpoint)
              (tla+-set-tlc-flag 'tla+-tlc-recover (cadr tla+/tlc-last-checkpoint))
            (tla+-set-tlc-flag 'tla+-tlc-recover (car tla+/tlc-last-checkpoint)))
        (tla+-unset-tlc-flag 'tla+-tlc-recover))
      )))

(defcustom tla+-auto-recover t
  "some doc")
                         ...
                         (if (not (equal lst-constraint '()))
                             (seq-reduce
                              ...
                              (mapcar
                               ...
                               (mapcar (lambda (s) (concat s "\n"))
                                       lst-constraint)) "") "")))
                       (set-visited-file-name (concat default-directory confname)))))
                 "Get Configuration")
  (widget-insert " ")
  (widget-create 'push-button
                 :notify
